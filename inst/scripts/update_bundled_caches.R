...
# Locate repo root: the directory containing this script's inst/scripts/
find_repo_root <- function() {
  # When invoked via Rscript --file=
  ...
  if (length(file_arg) > 0) {
    ...
    # inst/scripts/ -> inst/ -> repo root
    root <- dirname(dirname(script_dir))
    if (dir.exists(file.path(root, "inst", "extdata"))) return(root)
  }
  # Fallback: cwd must be the repo root
  wd <- normalizePath(getwd(), winslash = "/", mustWork = FALSE)
  if (dir.exists(file.path(wd, "inst", "extdata"))) return(wd)
  stop("Cannot locate repo root. Run from the repo root or via Rscript inst/scripts/update_bundled_caches.R")
}
...
r_dir       <- file.path(repo_root, "glossary", "R")   # canonical shared R source
...
for (f in c("utils.R", "data_ipbes.R", "data_ipcc.R", "similarity_text.R",
            "data_merge.R", "mod_table.R", "hierarchy_terms.R",
            "mod_graph.R", "ipcc_report_names.R")) {
...
expected_merged_meta <- list(schema = 1L, ipbes_md5 = ipbes_md5, ipcc_md5 = ipcc_md5)
...
saveRDS(list(meta = expected_merged_meta, merged = merged), merged_cache_path)
cat(sprintf("Saved %s (%d rows)\n", merged_cache_path, nrow(merged)))

hier_edges <- compute_term_hierarchy(merged_data = merged, min_score = 0, best_parent_only = FALSE)
saveRDS(list(meta = .hierarchy_cache_meta(merged), edges = hier_edges), hier_cache_path)
cat(sprintf("Saved %s (%d edges)\n", hier_cache_path, nrow(hier_edges)))

# Sync updated caches to app subdirectories
sync_files <- c("merged_glossary_cache.rds", "hierarchy_edges_cache.rds",
                "ipbes_glossary.csv", "ipcc_glossary.csv")
for (dir in c(file.path(repo_root, "compare", "inst", "extdata"),
              file.path(repo_root, "glossary", "inst", "extdata"))) {
  for (f in sync_files) {
    src <- file.path(extdata_dir, f)
    if (file.exists(src)) {
      file.copy(src, file.path(dir, f), overwrite = TRUE)
