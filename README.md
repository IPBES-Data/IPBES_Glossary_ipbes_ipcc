...
| IPCC Glossary (Multilingual Matrix) | https://apps.ipcc.ch/glossary/ | `inst/extdata/ipcc_glossary_multilingual.csv` (generated by `data-raw/prepare_data.R`) |
| Unified Multilingual Master (IPBES + IPCC) | IPBES CSV + IPCC live endpoints | `inst/extdata/glossary_multilingual_parquet/` (canonical parquet, partitioned by `source/language/report_or_assessment`) |
| Unified Multilingual Runtime Cache | generated from canonical parquet | `inst/extdata/glossary_multilingual_runtime.rds` |
...
This runs the IPCC scrapers (takes several minutes) and writes:

- `inst/extdata/ipcc_glossary.csv` (English report summary rows)
- `inst/extdata/ipcc_glossary_multilingual.csv` (language matrix for `en, ar, es, fr, ru, zh`)
- `inst/extdata/glossary_multilingual_parquet/` (canonical unified multilingual dataset)
- `inst/extdata/glossary_multilingual_runtime.rds` (runtime cache used by app wiring)
- `inst/extdata/glossary_multilingual_unified.csv` (inspection/back-compat export)

Multilingual rows are persisted as a full `(id, report, language)` matrix.
When a translation is not available, a placeholder row is still written with
status metadata.
