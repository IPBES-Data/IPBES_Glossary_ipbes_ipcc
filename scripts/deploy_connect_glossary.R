# Deploy helper for Posit Connect / Connect Cloud (glossary explorer app)
#
...
base_name <- "glossary-ipbes-ipcc-explorer"
...
known_prod_guids <- resolve_known_prod_guids(
  c(trimws(Sys.getenv("CONNECT_PROD_GUID", "")), trimws(Sys.getenv("CONNECT_PROD_GUID_GLOSSARY", "")))
)
